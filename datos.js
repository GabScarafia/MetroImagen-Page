const datosWeb = {
  empresa: {
    nombre: "Metro imagen",
    whatsapp: "+54 9 11 3494-0132",
    email: "info@metroimagen.com",
    descripcionNosotros: "Con más de 30 años de experiencia en la industria diseñamos, fabricamos e instalamos letreros, marquesinas y estructuras publicitarias que responden a las necesidades de cada cliente.",
    mediaNosotros: [
      // Ejemplo de cómo enlazar fotos locales:
      "img/nosotros/foto1.jpg",
      "img/nosotros/foto2.jpg"
      // Si tuvieras un video, lo agregas así: "img/nosotros/mivideo.mp4"
    ]
  },
  productos: [
    {
      id: "letras",
      titulo: "LETRAS",
      portada: "img/productos/letras/portada.jpg",
      descripcion: "Letras corpóreas de distintos materiales y acabados.",
      media: [
        "img/productos/letras/foto1.jpg",
        "img/productos/letras/foto2.jpg"
      ]
    },
    {
      id: "fronts",
      titulo: "FRONTS",
      portada: "img/productos/fronts/portada.jpg",
      descripcion: "Frentes comerciales atractivos y duraderos.",
      media: [
        "img/productos/fronts/foto1.jpg",
        "img/productos/fronts/foto2.jpg"
      ]
    },
    {
      id: "marquesinas",
      titulo: "MARQUESINAS",
      portada: "img/productos/marquesinas/portada.jpg",
      descripcion: "Marquesinas de alto impacto.",
      media: [
        "img/productos/marquesinas/foto1.jpg",
        "img/productos/marquesinas/foto2.jpg"
      ]
    },
    {
      id: "carteles-backlight",
      titulo: "CARTELES BACKLIGHT",
      portada: "img/productos/carteles-backlight/portada.jpg",
      descripcion: "Carteles luminosos backlight para noche y día.",
      media: [
        "img/productos/carteles-backlight/foto1.jpg",
        "img/productos/carteles-backlight/foto2.jpg"
      ]
    },
    {
      id: "pastillas",
      titulo: "PASTILLAS",
      portada: "img/productos/pastillas/portada.jpg",
      descripcion: "Carteles tipo pastilla, ideales para salientes.",
      media: [
        "img/productos/pastillas/foto1.jpg",
        "img/productos/pastillas/foto2.jpg"
      ]
    },
    {
      id: "vinilo-plotter",
      titulo: "VINILO/PLOTTER",
      portada: "img/productos/vinilo-plotter/portada.jpg",
      descripcion: "Ploteos y vinilos decorativos o institucionales.",
      media: [
        "img/productos/vinilo-plotter/foto1.jpg",
        "img/productos/vinilo-plotter/foto2.jpg"
      ]
    },
    {
      id: "carteles-columna",
      titulo: "CARTELES CON COLUMNA",
      portada: "img/productos/carteles-columna/portada.jpg",
      descripcion: "Carteles de gran altura con estructura de columna.",
      media: [
        "img/productos/carteles-columna/foto1.jpg",
        "img/productos/carteles-columna/foto2.jpg"
      ]
    },
    {
      id: "carteles-pie-calle",
      titulo: "CARTELES CON PIE PARA CALLE",
      portada: "img/productos/carteles-pie-calle/portada.jpg",
      descripcion: "Pizarras o carteles portátiles para la vereda.",
      media: [
        "img/productos/carteles-pie-calle/foto1.jpg",
        "img/productos/carteles-pie-calle/foto2.jpg"
      ]
    }
  ]
};
